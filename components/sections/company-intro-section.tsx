...
const highlightsData = [
  { value: "150+", labelKey: "companyIntro.stats1" },
  { value: "50+", labelKey: "companyIntro.stats2" },
  { value: "98%", labelKey: "companyIntro.stats3" },
]
          ...
          <div className="mt-7 grid gap-3">
            {pillarsData.map((pillarKey, index) => (
              ...
              >
                ...
                <p className="text-sm font-medium text-foreground md:text-base">{t(pillarKey)}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-8 grid max-w-2xl grid-cols-1 gap-5 sm:grid-cols-3">
            {highlightsData.map((item, index) => (
              ...
              >
                ...
                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{t(item.labelKey)}</p>
              </div>
            ))}
          </div>
        ...
        >
          <div className="relative mx-auto max-w-md">
            ...
            <div className="absolute -left-8 bottom-8 max-w-[220px] rounded-2xl border border-border/50 bg-background/90 p-4 shadow-xl backdrop-blur-xl">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-accent">
                {t("companyIntro.focusLabel")}
              </p>
              <p className="mt-2 text-sm leading-relaxed text-foreground">
                {t("companyIntro.focus")}
              </p>
            </div>
          </div>
        </div>
