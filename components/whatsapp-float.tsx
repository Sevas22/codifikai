"use client"

/**
 * Botón flotante de WhatsApp: el cierre de negocio principal del sitio.
 *
 * Sustituye al panel de contacto de tres opciones y al widget de agente IA.
 * Un solo toque lleva a la conversación con el mensaje ya escrito; el correo y
 * el teléfono siguen disponibles en el footer y en /contact para quien los
 * prefiera.
 *
 * El glifo va como SVG en línea a propósito: traerlo de FontAwesome por CDN
 * añadiría una petición bloqueante y una dependencia externa para un icono.
 */

import * as React from "react"
import { usePathname } from "next/navigation"

import { useLanguage } from "@/components/providers/language-provider"
import { WHATSAPP_URL } from "@/lib/contact"
import { cn } from "@/lib/utils"

const LABEL = {
  es: "Escríbenos por WhatsApp",
  en: "Message us on WhatsApp",
} as const

function WhatsAppGlyph({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      className={className}
      aria-hidden
      focusable="false"
    >
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.174.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.247-.694.247-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884a9.82 9.82 0 0 1 6.988 2.896 9.83 9.83 0 0 1 2.893 6.994c-.003 5.45-4.437 9.886-9.885 9.886m8.413-18.297A11.82 11.82 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.548 4.142 1.588 5.945L.057 24l6.305-1.654a11.88 11.88 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.82 11.82 0 0 0 20.464 3.488" />
    </svg>
  )
}

export function WhatsAppFloat({ className }: { className?: string }) {
  const pathname = usePathname()
  const { language } = useLanguage()

  // El panel administrativo es una herramienta interna: el botón de captación
  // de clientes no tiene nada que hacer ahí.
  if (pathname.startsWith("/admin")) return null

  const label = LABEL[language] ?? LABEL.es

  return (
    <a
      href={WHATSAPP_URL}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={label}
      data-gtm="whatsapp-float"
      className={cn(
        // Círculo exacto: medidas fijas y sin hueco interno que lo deforme.
        // La versión anterior era una píldora que se ensanchaba al pasar el
        // cursor, y el espacio reservado para el texto la sacaba de redonda.
        "group fixed z-50 grid h-14 w-14 place-items-center sm:h-16 sm:w-16",
        "right-[max(1rem,env(safe-area-inset-right,0px))]",
        "bottom-[max(1.25rem,env(safe-area-inset-bottom,0px))]",
        "rounded-full bg-[#25D366] text-white",
        "shadow-[0_8px_24px_-6px_rgba(37,211,102,0.55)]",
        "transition-[transform,box-shadow] duration-300 ease-out",
        "hover:-translate-y-0.5 hover:shadow-[0_14px_34px_-8px_rgba(37,211,102,0.7)]",
        "focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#25D366]",
        "motion-reduce:transition-none motion-reduce:hover:translate-y-0",
        className
      )}
    >
      {/* Anillo de pulso: llama la atención sin tapar contenido. No recibe
          clics: al crecer invade el botón de sonido que va encima y le
          robaba el clic para abrir WhatsApp. */}
      <span
        className="pointer-events-none absolute inset-0 -z-10 animate-ping rounded-full bg-[#25D366]/40 motion-reduce:animate-none"
        style={{ animationDuration: "2.4s" }}
        aria-hidden
      />
      <WhatsAppGlyph className="h-7 w-7 transition-transform duration-300 group-hover:scale-110 motion-reduce:transition-none" />
    </a>
  )
}
